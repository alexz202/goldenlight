...
class Extended extends Base
{
    ...
    public function putInTube($tube, $data, $options = null,$reset=false)
    {
      ...
      $jobid= parent::put($data, $options);
        ...
        if($reset)
        $sqlvalue=array(
            $jobid,$data,$tube,json_encode($options),1
        );
        else
            $sqlvalue=array(
                $jobid,$data,$tube,json_encode($options),0
            );
    ...
    */
            ...
            default:
                ...
                if ($result != 0) {
                    ...
                    try {
                        ...
                        $id=$job->getId();
                        $job->delete();
                        $sql="DELETE FROM `work` where jobId=$id";
    ...
    public function getmissworkbydb(){
        $connection=$this->setDB();
        $sql='select * from work where reset=0 ';
       $res= $connection->fetchAll($sql,DB::FETCH_ASSOC);
        $connection->close();
        return $res;
    }
